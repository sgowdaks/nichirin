...
def preprocess_query(query, logger=None):
    clean_query = (
        query.replace('"', '').replace("'", "").replace("?", "").replace("!", "").replace(".", "").replace(",", "")
    )
    if logger:
...
def clean_data(text):
    ...
    text = re_sub(r"{}{}[)dD]+|[)dD]+{}{}".format(emoji_eyes, emoji_nose, emoji_nose, emoji_eyes), "<smile>", text)
    text = re_sub(r"{}{}p+".format(emoji_eyes, emoji_nose), "<lolface>", text)
    text = re_sub(r"{}{}\(+|\)+{}{}".format(emoji_eyes, emoji_nose, emoji_nose, emoji_eyes), "<sadface>", text)
    ...
    if not os.path.exists(args['json_file_path'] + ".tsv_"):
        if type_ == ".json":
            ...
            if not os.path.isdir(path):
                bash_command = (
                    "split -a 4 -d --additional-suffix=.tsv -l 1000000 " + json_file_path + ".tsv_ " + path + "/part-"
                )
...
def parse_args():
    ...
    parser.add_argument('--file_type', type=str, default=".json", help='Give the file type either .tsv or .json')
    parser.add_argument(
        '--json_file_path',
        type=str,
